...
function in = configure(in)
	...
	in.file = sprintf('t_%04d_%03d.mat', ...
 	...
 	obs = { ...
 		...
 		'<V(x)>/L' @(a,r) 4*r.c.gamma*r.c.n^2*r.observe{5}(a,r); ...
 		'N/L (x)' @(a,r) xint(r.observe{3}(a,r), r) / L; ...
 		'N/L (k)' @(a,r) xint(r.observe{4}(a,r), r.dk, r) / L; ...
 		'<T(k)>/L' @(a,r)  xint(r.observe{6}(a,r), r.dk, r) / L; ...
...
%	in.transforms{length(in.olabels)} = [true false];
%	in.gfunction{length(in.olabels)} = @(data, ~) abs(data);
%	in.transforms{length(in.olabels) - 1} = [true false];
%	in.gfunction{length(in.olabels) - 1} = @(data, ~) abs(data);
	in.transforms{length(in.olabels)} = [true false];
	in.function{length(in.olabels)} = @(data, ~) abs(data{length(in.olabels)});
	in.transforms{length(in.olabels) - 1} = [true false];
	in.function{length(in.olabels) - 1} = @(data, ~) abs(data{length(in.olabels) - 1});
